...
if ($action === 'like') {
    ...
    if ($result_like->num_rows > 0) {
        ...
        try {
            ...
            $update_event = $conn->prepare("UPDATE events SET LikeCount = LikeCount + 1 WHERE EventID = ?");
    ...
    if ($result_dislike->num_rows > 0) {
        ...
        try {
            ...
            $update_event = $conn->prepare("UPDATE events SET DislikeCount = DislikeCount + 1 WHERE EventID = ?");
            $update_event->bind_param("i", $event_id);
            $update_event->execute();
